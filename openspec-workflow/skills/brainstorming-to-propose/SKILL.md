---
name: brainstorming-to-propose
description: "将 brainstorming 的需求探索结论自动流转为 OpenSpec 正式提案。当项目存在 OpenSpec 时，将 brainstorming 的结论直接转化为 /opsx:propose 的输入，不写独立的 design doc。触发场景：用户提出新功能开发需求、新页面需求、新模块需求，或任何需要从想法到正式提案的场景。关键词：'新功能'、'新页面'、'新模块'、'需求开发'、'做个 xxx'。即使没有明确提到，只要是功能开发的初期阶段，都应主动使用。"
---
...
/openspec-workflow:brainstorming-to-propose
...
brainstorming 进入以下状态时，视为阶段 1 即将结束：
- 需求目标已澄清
- 方案已比较并选出推荐项
- 用户对推荐方向无异议
...
只有用户明确回复"确认"/"继续"/"是"等肯定语义后，才进入阶段 2。
用户提出修改则继续阶段 1 澄清，再次收敛后重新触发确认提示。
...
| 行为规格 | 用户可见的行为、边界条件、错误处理 | spec.md |
...
用户确认后，先整理以下结构化摘要，作为 `/opsx:propose` 的显式输入：

1. **change-name**：基于功能目标生成 kebab-case 的 change 名称
2. **proposal 内容**：目标、范围、影响
3. **spec 内容**：行为规格、边界条件、错误处理
4. **design 内容**：技术选型、数据模型、接口设计
5. **tasks 雏形**：按阶段/模块拆分的实现步骤

本 skill 不直接写 OpenSpec 文件，只负责准备输入。

### 阶段 3：/opsx:propose（自动流转）

阶段 2 的结构化摘要整理完成后，调用 `/opsx:propose` 并将摘要作为上下文传入。

由 `/opsx:propose` 负责在 `openspec/changes/<change-name>/` 下生成：
- `proposal.md` — 需求描述（Why/What/Impact）
- `spec.md` — 行为规格（用户可见行为、边界条件、错误处理）
- `design.md` — 技术设计决策
- `tasks.md` — 任务清单（带 checkbox）

文件生成由 `/opsx:propose` 统一完成。
...
如果项目未初始化 OpenSpec，本 skill 仅执行阶段 1 的需求澄清与方案比较，不进入后续阶段，也不生成任何正式文档。

用户确认方向后，应提示：

> 项目未检测到 OpenSpec。建议先初始化 OpenSpec，或选择其他方式落地设计。

如果用户要求直接生成 design doc，再回退到 `superpowers:brainstorming` 的完整默认流程。
...
| `/opsx:propose` | 被调用，阶段 3 使用，创建正式 artifact |
